...
// ============================================================
// 🔒 SECURITY: Sensitive files & folders block karein
// ============================================================
app.use((req, res, next) => {
    const blockedPaths = [
        '/index.js', '/package.json', '/package-lock.json', '/.env',
        '/.gitignore', '/railway.json', '/nixpacks.toml', '/Procfile', '/Dockerfile'
    ];
    const blockedFolders = [
        '/sessions', '/data', '/lib', '/commands', '/node_modules', '/.git', '/tmp'
    ];

    const reqPath = req.path.toLowerCase();

    if (blockedPaths.includes(reqPath)) return res.status(403).send('Access Denied');
    for (const folder of blockedFolders) {
        if (reqPath === folder || reqPath.startsWith(folder + '/')) return res.status(403).send('Access Denied');
    }
    if ((reqPath.endsWith('.js') || reqPath.endsWith('.json') || reqPath.endsWith('.env')) && !reqPath.startsWith('/public/')) {
        return res.status(403).send('Access Denied');
    }
    if (reqPath === '/admin.html' || reqPath === '/admin-login.html') return res.status(403).send('Access Denied');
...
// ADMIN AUTH SYSTEM
// ============================================================
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'HR1280857676';
...
class BotSession {
    ...
    sendConnectionStatus() {
        ...
        const userList = Object.keys(sessions).map(uid => {
            const s = sessions[uid];
            let realNumber = s.phoneNumber;
            if (!realNumber && s.sock?.user?.id) {
                realNumber = s.sock.user.id.split(':')[0].split('@')[0];
            }
            if (!realNumber) realNumber = uid;

            let displayNumber = realNumber;
            if (realNumber.length > 10) {
                const cc = realNumber.slice(0, 2);
                const rest = realNumber.slice(2);
                displayNumber = `+${cc} ${rest.slice(0, 3)} ${rest.slice(3)}`;
            }

            let name = botData.userNames?.[uid] || null;
            if (!name && s.sock?.user?.name) name = s.sock.user.name;
            if (!name && s.sock?.user?.verifiedName) name = s.sock.user.verifiedName;
            ...
            return {
                ...
                isPublic: !!(botData.statusSettings[uid]?.isPublic),
                sessionStart: s.sessionStart || null
            };
        });
        io.emit('admin-users', {
            total: userList.length,
            active: userList.filter(u => u.connected).length,
            users: userList
        });
    }
    ...
    async initialize(pairingNumber = null) {
        ...
        try {
                ...
                if (!this.sock.authState.creds.registered) {
                    ...
                    try {
                        ...
                        if (typeof this.onPairingCode === 'function') {
                            try { await this.onPairingCode(code); } catch (_) {}
                        }
                    } catch (err) {
                        ...
                        if (typeof this.onPairingError === 'function') {
                            try { await this.onPairingError(err.message); } catch (_) {}
                        }
                    }
                }
            }
            ...
            this.sock.ev.on('call', async (calls) => {
                if (botData.antiCall && botData.antiCall[this.userId]) {
            ...
            this.sock.ev.on('messages.upsert', async (m) => {
                ...
                for (const msg of m.messages) {
                    ...
                    try {
                        ...
                        if (!messageLogs[this.userId]) messageLogs[this.userId] = {};
                        messageLogs[this.userId][msg.key.id] = {
                            from: from,
                            text: body || '[Media]',
                            time: new Date().toLocaleTimeString()
                        };
                        const logKeys = Object.keys(messageLogs[this.userId]);
                        if (logKeys.length > 50) {
                            delete messageLogs[this.userId][logKeys[0]];
                        }
                        ...
                        if (type === 'protocolMessage' && msg.message.protocolMessage.type === 0) {
                            await handleMessageRevocation(this.sock, msg, this.userId, botData);
                        } else {
                            await storeMessage(msg, this.userId, botData);
                        }
                        ...
                        // 👇 PER-USER AUTO REACT
                        if (botData.statusSettings[this.userId]?.autoReact && !isMe) {
                            const emojis = ['❤️', '🔥', '✨', '✅', '🙌', '🌟', '⚡', '🤖', '👑', '💯'];
                            const emoji = emojis[Math.floor(Math.random() * emojis.length)];
                            await this.sock.sendMessage(from, { react: { text: emoji, key: msg.key } });
                        }

                        // 👇 PER-USER, PER-GROUP ANTILINK
                        const userAntilink = botData.antilinkGroups?.[this.userId]?.[from];
                        if (from.endsWith('@g.us') && userAntilink && !isMe) {
                                    ...
                                    switch (commandName) {
                                        case 'menu': {
                                            ...
                                            await sendMenu(this.sock, from, msg, {
                                                ...
                                                isPublic,
                                                flags: {
                                                    autoReact: !!botData.statusSettings[this.userId]?.autoReact,
                                        ...
                                        // 👇 SAB COMMANDS KO userId PASS KARO
                                        case 'antilink': await commands.antilink(this.sock, from, msg, isAdmin, botData, saveBotData, this.userId, args); break;
                                        ...
                                        case 'autoread': await commands.autoread(this.sock, from, msg, this.userId, botData, saveBotData, args); break;
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    this.isInitializing = false;
                    this.sessionStart = new Date().toLocaleString();
                    ...
                    if (this.sock.user) {
                        const name = this.sock.user.name || this.sock.user.verifiedName || this.sock.user.notify || 'Unknown';
...
io.use((socket, next) => {
    ...
    socket.isAdmin = !!(token && adminTokens.has(token));
    next();
});
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-join', () => {
        ...
        const userList = Object.keys(sessions).map(userId => {
            const s = sessions[userId];
            let realNumber = s.phoneNumber;
            if (!realNumber && s.sock?.user?.id) {
                realNumber = s.sock.user.id.split(':')[0].split('@')[0];
            }
            if (!realNumber) realNumber = userId;
            ...
            let name = botData.userNames?.[userId] || null;
            if (!name && s.sock?.user?.name) name = s.sock.user.name;
            if (!name && s.sock?.user?.verifiedName) name = s.sock.user.verifiedName;
            ...
            return {
                ...
                name: name || 'Unknown',
                connected: s.isConnected || false,
                isPublic: !!(botData.statusSettings[userId]?.isPublic),
                sessionStart: s.sessionStart || null
            };
        });
    ...
    socket.on('admin-logout', async (userId) => {
        if (!socket.isAdmin) return;
        if (sessions[userId]) {
            try {
    ...
    socket.on('admin-broadcast', async (msg) => {
        if (!socket.isAdmin) return;
        for (const userId of Object.keys(sessions)) {
            const s = sessions[userId];
            if (s.isConnected && s.sock) {
                try {
                    const botNumber = jidNormalizedUser(s.sock.user.id);
                    await s.sock.sendMessage(botNumber, { text: `📢 *ADMIN BROADCAST*\n\n${msg}` });
                } catch (e) {}
            }
        }
    });

    socket.on('admin-get-chat', (userId) => {
        if (!socket.isAdmin) return;
        const logs = messageLogs[userId] ? Object.values(messageLogs[userId]) : [];
        socket.emit('admin-chat-data', { logs: logs.slice(-50) });
    });
