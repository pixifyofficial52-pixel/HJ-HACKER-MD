...
app.use((req, res, next) => {
    ...
    // Blocked exact paths
    if (blockedPaths.includes(reqPath)) {
        return res.status(403).send('Access Denied');
    }

    // Blocked folders
    for (const folder of blockedFolders) {
        if (reqPath === folder || reqPath.startsWith(folder + '/')) {
            return res.status(403).send('Access Denied');
        }
    }

    // Block direct .js, .json, .env files (except public folder)
    if (
        (reqPath.endsWith('.js') || reqPath.endsWith('.json') || reqPath.endsWith('.env')) &&
        !reqPath.startsWith('/public/')
    ) {
        return res.status(403).send('Access Denied');
    }

    // Block admin files from direct access (only through /admin route with token)
    if (reqPath === '/admin.html' || reqPath === '/admin-login.html') {
        return res.status(403).send('Access Denied');
    }
...
// 👇 Static files sirf public folder se serve karein
