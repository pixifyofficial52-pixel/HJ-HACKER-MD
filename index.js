...
try {
    if (fs.existsSync(commandsPath)) {
        ...
        for (const file of files) {
            ...
            try {
                const loaded = require(`./commands/${cmdName}`);
                // Handle both direct function and object exports
                if (typeof loaded === 'function') {
                    commands[cmdName] = loaded;
                } else if (typeof loaded === 'object') {
                    // For commands like autoread, translate that export object
                    commands[cmdName] = loaded[cmdName + 'Command'] || 
                                       loaded['handle' + cmdName.charAt(0).toUpperCase() + cmdName.slice(1) + 'Command'] ||
                                       loaded[Object.keys(loaded)[0]];
                }
                console.log(`✅ Loaded: ${cmdName}`);
            } catch (e) {
                console.log(`⚠️ Failed to load ${cmdName}: ${e.message}`);
            }
        }
    }
} catch (e) {
    console.log('⚠️ Command loading error:', e.message);
}
        ...
        for (const userId of authDirs) {
            ...
            if (stats.isDirectory()) {
                ...
                if (fs.existsSync(credsFile)) {
                    console.log(`[System] Found existing session for: ${userId}`);
                    if (!sessions[userId]) {
                        sessions[userId] = new BotSession(userId);
...
class BotSession {
    ...
    buildMenu() {
        ...
        const groups = {
            'DOWNLOAD': ['song', 'video', 'insta', 'instagram', 'tiktok', 'facebook', 'apk', 'gdrive', 'mf'],
            ...
            'OWNER': ['owner', 'ping', 'private', 'public', 'autoread', 'autoreacts', 'antidelete', 'anticall', 'status'],
            ...
            this.sock.ev.on('messages.upsert', async (m) => {
                ...
                for (const msg of m.messages) {
                    ...
                    try {
                        ...
                        // Auto features (optional, wrapped in try-catch)
                        try {
                        ...
                        if (body && body.startsWith(prefix)) {
                            ...
                            if (commands[commandName]) {
                                try {
                                    ...
                                    } else if (['accept'].includes(commandName)) {
                                        await cmdFunc(this.sock, from, msg, isAdmin);
                                    } else if (['chid'].includes(commandName)) {
                                ...
                                this.sendLog(`Unknown command: ${commandName}`, 'warning');
                            }
                        }
                    } catch (e) {
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    if (statusCode === DisconnectReason.loggedOut) {
                        this.sendLog('Logged out. Clearing auth...', 'error');
                    ...
                    this.sendLog(`Bot online.`, 'success');
