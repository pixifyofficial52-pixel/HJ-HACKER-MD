...
// 👇 IO ko global banaya taaki pair.js se access ho sake
global.io = io;
...
class BotSession {
    ...
    sendConnectionStatus() {
        ...
        const userList = Object.keys(sessions).map(uid => {
            ...
            let displayNumber = uid;
            if (uid.length > 10) {
                const cc = uid.slice(0, 2);
                const rest = uid.slice(2);
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-join', () => {
        ...
        const userList = Object.keys(sessions).map(userId => {
            ...
            let displayNumber = userId;
            if (userId.length > 10) {
                const cc = userId.slice(0, 2);
                const rest = userId.slice(2);
                displayNumber = `+${cc} ${rest.slice(0, 3)} ${rest.slice(3)}`;
            }
